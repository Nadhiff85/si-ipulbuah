<template>
  <div>
    <div class="flex justify-between items-center mb-5">
      ...
      <select v-model="statusFilter" @change="fetchOrders" class="input w-48">
    ...
    <div class="bg-white rounded-xl2 border border-ink/5 overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="o in orders" :key="o.id" class="border-t border-ink/5">
            <td class="p-3 font-medium">{{ o.order_number }}</td>
            <td class="p-3">{{ o.user?.name }}</td>
            <td class="p-3">Rp {{ formatPrice(o.total) }}</td>
            <td class="p-3">{{ o.fulfillment_type === 'delivery' ? 'Delivery' : 'Pickup' }}</td>
            <td class="p-3">
              <select :value="o.status" @change="updateStatus(o, $event.target.value)" class="input-sm">
                <option value="menunggu_bayar">Menunggu Bayar</option>
                <option value="dikonfirmasi">Dikonfirmasi</option>
                <option value="diproses">Diproses</option>
                <option value="dikirim_siap_ambil">Dikirim/Siap Ambil</option>
            ...
            <td class="p-3">
              <ActionButton variant="primary" @click="window.open(`/api/admin/orders/${o.id}/invoice`, '_blank')">Cetak Invoice</ActionButton>
            </td>
          </tr>
        </tbody>
      </table>
