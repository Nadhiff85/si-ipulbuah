<template>
  <div>
    <h2 class="font-bold text-ink mb-2">Audit Trail Pelanggan</h2>
    <p class="text-ink/60 text-sm mb-5">Log aktivitas lengkap untuk keamanan, deteksi anomali & kepatuhan UU PDP</p>
    ...
    <div class="grid md:grid-cols-3 gap-5">
      <div class="bg-white rounded-xl2 border border-ink/5 p-4">
        <p class="font-semibold text-sm mb-3">Daftar Pelanggan</p>
        <button
          v-for="c in customers"
          :key="c.id"
          @click="selectCustomer(c)"
          class="w-full text-left px-3 py-2 rounded-lg text-sm mb-1"
          :class="selected?.id === c.id ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-ink/5'"
        >
          {{ c.name }}
        </button>
      </div>
      ...
      <div v-if="trail" class="md:col-span-2 space-y-4">
        <div class="bg-white rounded-xl2 border border-ink/5 p-4">
          <p class="font-semibold text-sm mb-2">🔐 Login Terakhir</p>
        ...
        <div class="bg-white rounded-xl2 border border-ink/5 p-4">
          <p class="font-semibold text-sm mb-2">👀 Riwayat Browsing</p>
        ...
        <div class="bg-white rounded-xl2 border border-ink/5 p-4">
          <p class="font-semibold text-sm mb-2">🛒 Pesanan & Pembayaran</p>
        ...
        <div class="bg-white rounded-xl2 border border-ink/5 p-4">
          <p class="font-semibold text-sm mb-2">📝 Aktivitas Lainnya</p>
