<template>
  <div class="max-w-3xl mx-auto px-4 py-12">
    ...
    <div v-else class="space-y-3">
      <details v-for="faq in faqs" :key="faq.id" class="glass-card rounded-xl2 p-4 group">
        <summary class="font-medium text-ink cursor-pointer flex justify-between items-center">
          {{ faq.question }}
          <ChevronDownIcon class="w-4 h-4 text-ink/40 group-open:rotate-180 transition-transform" stroke-width="1.75" />
        </summary>
        <p class="text-ink/60 text-sm mt-3">{{ faq.answer }}</p>
      </details>
    </div>

    <div class="mt-8 glass-card-soft bg-primary/5 rounded-xl2 p-5 text-center">
      <p class="text-sm text-ink/70 mb-2">Masih ada pertanyaan lain?</p>
      <a :href="storeInfo.whatsappUrl" target="_blank" class="inline-flex items-center gap-2 text-success font-medium hover:underline transition-colors cursor-pointer">
