<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    <h1 class="text-xl font-bold text-ink mb-1">🎁 Buat Parsel Kustom</h1>
        ...
        <div class="bg-white rounded-xl2 border border-ink/5 p-5">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <label
              v-for="p in products"
              :key="p.id"
              class="border rounded-xl2 p-3 text-sm cursor-pointer"
              :class="selectedProducts[p.id] ? 'border-primary bg-primary/5' : 'border-ink/10'"
            >
              <div class="flex justify-between items-center mb-1">
                <span class="font-medium">{{ p.name }}</span>
                <input type="checkbox" :checked="!!selectedProducts[p.id]" @change="toggleProduct(p)" class="accent-primary" />
              </div>
              ...
              <div v-if="selectedProducts[p.id]" class="flex items-center gap-2">
                <button @click.prevent="changeQty(p, -0.5)" class="w-6 h-6 border rounded-full text-xs">-</button>
        ...
        <div class="bg-white rounded-xl2 border border-ink/5 p-5">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <button
              ...
              class="border-2 rounded-xl2 p-3 text-sm text-left"
              :class="selectedContainer?.id === c.id ? 'border-primary bg-primary/5' : 'border-ink/10'"
            >
        ...
        <div class="bg-white rounded-xl2 border border-ink/5 p-5">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <button
              ...
              class="border-2 rounded-xl2 p-3 text-sm text-left"
              :class="!selectedCard ? 'border-primary bg-primary/5' : 'border-ink/10'"
            >
              Tanpa Kartu
            </button>
            <button
              ...
              class="border-2 rounded-xl2 p-3 text-sm text-left"
              :class="selectedCard?.id === c.id ? 'border-primary bg-primary/5' : 'border-ink/10'"
            >
              <p class="font-medium">{{ c.name }}</p>
              <p class="text-ink/50 text-xs">+Rp {{ formatPrice(c.extra_price) }}</p>
            </button>
          ...
          <textarea
            ...
            class="w-full border border-ink/15 rounded-lg px-3 py-2 text-sm mt-3"
          ></textarea>
        </div>
      </div>
