<template>
  <div v-if="product" class="max-w-6xl mx-auto px-4 py-8 grid md:grid-cols-2 gap-8">
    <!-- ===== Galeri Foto ===== -->
    <div>
      <div class="aspect-square bg-primary/5 rounded-xl2 overflow-hidden flex items-center justify-center text-8xl mb-3">
    ...
    <div>
      ...
      <p v-if="wholesale" class="text-sm text-primary font-medium mb-4">
        🏷️ Harga grosir Rp {{ formatPrice(wholesale.price) }}/{{ product.unit }} — min. beli {{ wholesale.min_qty }} {{ product.unit }}
      </p>

      <!-- Estimasi Kesegaran -->
      <div v-if="freshnessRemaining !== null" class="bg-success/10 border border-success/30 rounded-xl2 p-3 mb-4 text-sm">
        🌿 Segar hingga <strong>{{ freshnessRemaining }} hari</strong> ke depan
      </div>
      ...
      <div v-if="product.variants?.length" class="mb-4">
        ...
        <div class="flex gap-2">
          <button
            ...
            :class="selectedVariant?.id === v.id ? 'bg-primary text-white border-primary' : 'border-ink/15 text-ink/70'"
          >
      ...
      <div class="flex items-center gap-3 mb-3">
        ...
        <div class="flex items-center border border-ink/15 rounded-full overflow-hidden">
          <button @click="qty > 1 && qty--" class="px-3 py-1.5 hover:bg-ink/5">-</button>
          <span class="px-4">{{ qty }}</span>
          <button @click="qty++" class="px-3 py-1.5 hover:bg-ink/5">+</button>
        </div>
      </div>
      <textarea
        v-model="note"
        ...
        class="w-full border border-ink/15 rounded-lg px-3 py-2 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-primary/40"
      ></textarea>
      ...
      <div class="flex gap-3 mb-6">
        <button
          @click="addToCart"
          :disabled="adding"
          class="flex-1 bg-accent hover:bg-accent-light text-white font-semibold py-3 rounded-full transition disabled:opacity-60"
        >
          {{ adding ? 'Menambahkan...' : '🛒 Tambah ke Keranjang' }}
        </button>
        <button @click="toggleWishlist" class="w-12 h-12 rounded-full border border-ink/15 flex items-center justify-center hover:bg-danger/10">
          {{ inWishlist ? '❤️' : '🤍' }}
        </button>
      </div>

      <!-- Tips Penyimpanan -->
      <div v-if="product.storage_tips" class="bg-white border border-ink/5 rounded-xl2 p-4 text-sm text-ink/70 mb-6">
        <p class="font-semibold text-ink mb-1">💡 Tips Penyimpanan</p>
      ...
      <div>
        ...
        <div v-for="r in product.reviews" :key="r.id" class="border-t border-ink/5 py-3">
          ...
          <p v-if="r.admin_reply" class="text-xs bg-primary/5 rounded-lg p-2 mt-2 text-ink/60">
            💬 Balasan Toko: {{ r.admin_reply }}
          </p>
        </div>
      </div>
    </div>
